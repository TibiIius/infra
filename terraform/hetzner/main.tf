terraform {
  required_providers {
    hcloud = {
      source  = "hetznercloud/hcloud"
      version = "~> 1.45.0"
    }
  }
}

provider "hcloud" {
  token = var.hcloud_token
}

resource "hcloud_server" "control_plane" {
  count = var.control_plane_count

  name        = "${var.cluster_name}${count.index > 0 ? "-cp${count.index}" : ""}"
  image       = "ubuntu-22.04"
  server_type = var.server_type
  location    = var.location

  user_data = <<-EOT
    #!/bin/bash
    set -euo pipefail
    TALOS_VERSION="${talos_version}"
    curl -fsSL "https://github.com/siderolabs/talos/releases/download/v\${TALOS_VERSION}/talosctl-linux-amd64" -o /usr/local/bin/talosctl
    chmod +x /usr/local/bin/talosctl
    TALOS_DIR="/etc/talos"
    mkdir -p "\${TALOS_DIR}"
    talosctl gen config talos-vps "https://\${PRIVATE_IP}:6443" "\${TALOS_DIR}"
    talosctl install node --config "\${TALOS_DIR}/machine-config.yaml"
    systemctl reboot
  EOT

  private_net {
    network_id = hcloud_network.talos_network.id
  }

  labels = {
    environment = var.environment
    role        = "talos-control-plane"
  }
}

resource "hcloud_server" "worker" {
  count = var.worker_count

  name        = "${var.cluster_name}-wn${count.index + 1}"
  image       = "ubuntu-22.04"
  server_type = var.server_type
  location    = var.location

  user_data = <<-EOT
    #!/bin/bash
    set -euo pipefail
    TALOS_VERSION="${talos_version}"
    curl -fsSL "https://github.com/siderolabs/talos/releases/download/v\${TALOS_VERSION}/talosctl-linux-amd64" -o /usr/local/bin/talosctl
    chmod +x /usr/local/bin/talosctl
    TALOS_DIR="/etc/talos"
    mkdir -p "\${TALOS_DIR}"
    talosctl gen config talos-vps "https://\${PRIVATE_IP}:6443" "\${TALOS_DIR}"
    talosctl install node --config "\${TALOS_DIR}/machine-config.yaml"
    systemctl reboot
  EOT

  private_net {
    network_id = hcloud_network.talos_network.id
  }

  labels = {
    environment = var.environment
    role        = "talos-worker"
  }
}

resource "hcloud_network" "talos_network" {
  name     = "talos-network"
  ip_range = "10.0.0.0/16"
}

resource "hcloud_network_subnet" "talos_subnet" {
  network_id   = hcloud_network.talos_network.id
  type         = "cloud"
  network_zone = "eu-central"
  ip_range     = "10.0.0.0/24"
}

output "control_plane_ips" {
  value       = hcloud_server.control_plane[*].ipv4_address
  description = "Public IPv4 addresses of control plane nodes"
}

output "control_plane_private_ips" {
  value       = hcloud_server.control_plane[*].private_net[0].ip_address
  description = "Private IPv4 addresses of control plane nodes"
}

output "worker_ips" {
  value       = hcloud_server.worker[*].ipv4_address
  description = "Public IPv4 addresses of worker nodes"
}

output "worker_private_ips" {
  value       = hcloud_server.worker[*].private_net[0].ip_address
  description = "Private IPv4 addresses of worker nodes"
}

output "talos_endpoint" {
  value       = "https://${hcloud_server.control_plane[0].private_net[0].ip_address}:6443"
  description = "Talos Kubernetes API endpoint"
}
